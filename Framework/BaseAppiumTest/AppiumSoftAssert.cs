    ...
    public class AppiumSoftAssert : SoftAssert
    {
        ...
        public override bool AreEqual(string expectedText, string actualText, string softAssertName, string message = "")
        {
            ...
            if (!didPass)
            {
                ...
                {
                    AppiumUtilities.CaptureScreenshot(this.appiumTestObject.AppiumDriver, this.Log, this.appiumTestObject, this.TextToAppend(softAssertName));
                }

                if (AppiumConfig.GetSavePagesourceOnFail())
                {
                    AppiumUtilities.SavePageSource(this.appiumTestObject.AppiumDriver, this.Log, this.appiumTestObject, StringProcessor.SafeFormatter(" ({0})", this.NumberOfAsserts));
                }
